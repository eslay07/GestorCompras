...
def _nombre_archivo(numero: str | None, proveedor: str | None) -> str | None:
    """Construye un nombre amigable combinando número y proveedor."""

    numero = (numero or "").strip()
    proveedor = limpiar_proveedor(proveedor)
    if proveedor:
        proveedor = re.sub(r"[^\w\- ]", "_", proveedor)
        proveedor = re.sub(r"_+", "_", proveedor).strip("_")
    partes = [parte for parte in (numero, proveedor) if parte]
