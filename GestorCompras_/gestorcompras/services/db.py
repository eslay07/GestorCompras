...
# Caché en memoria para app_config (evita round-trips a DB en cada llamada)
...
def get_connection():
    """
    Retorna una conexión a la base de datos SQLite con WAL mode activado
    para mejor soporte de concurrencia.
    """
...
def init_db():
    ...
    try:
        ...
        # Tabla para tareas temporales — task_number UNIQUE para evitar duplicados
        cursor.execute("""
        ...
        """)
        # Migrar tabla existente si aún no tiene el constraint UNIQUE
        cursor.execute("PRAGMA table_info(tasks_temp)")
        cursor.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='tasks_temp'"
        )
        row = cursor.fetchone()
        if row and "UNIQUE" not in (row[0] or "").upper():
            cursor.execute("ALTER TABLE tasks_temp RENAME TO tasks_temp_old")
            cursor.execute("""
            ...
            """)
            cursor.execute(
                "INSERT OR IGNORE INTO tasks_temp (task_number, reasignacion, details) "
                "SELECT task_number, reasignacion, details FROM tasks_temp_old"
            )
            cursor.execute("DROP TABLE tasks_temp_old")
...
def get_supplier_by_ruc(ruc: str):
    """Retorna (email, email_alt) de un proveedor buscado directamente por RUC."""
    conn = get_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT email, COALESCE(email_alt, '') FROM suppliers WHERE ruc=?",
            (ruc,),
        )
        return cursor.fetchone()
    finally:
...
def insert_task_temp(task_number, reasignacion, details_dict):
    ...
    Evita duplicados usando INSERT OR IGNORE (atómico, sin race condition).
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR IGNORE INTO tasks_temp (task_number, reasignacion, details) VALUES (?, ?, ?)",
...
def get_config(key, default=None):
    """
    Retorna el valor asociado a la clave en app_config.
    Usa caché en memoria para evitar round-trips a DB en cada llamada.
