...
def read_report(creds_path: str, spreadsheet_id: str, sheet_name: str) -> List[Dict[str, str]]:
    ...
    and optionally ``Estado Aprobación`` when that column is present. Only rows
    con "Aprobado" en dicha columna serán devueltos.
    """
    ...
    estado_variants = ["estado aprobacion", "estado aprobación", "estado"]
    has_estado = len(headers) >= 6 and any(headers[5].startswith(v) for v in estado_variants)
        ...
        for row in data[1:]:
            if len(row) >= 5:
                ...
                if has_estado and len(row) >= 6:
                    item["Estado Aprobación"] = row[5].strip()
                result.append(item)
        if has_estado:
            result = [r for r in result if r.get("Estado Aprobación", "").strip().lower().startswith("apro")]
        return result
